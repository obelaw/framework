<div {{ $attributes->merge(['class' => 'mb-3 row']) }}>
    ...
    <div class="col">
        <div class="row g-2">
            <select class="form-select @error('models.' . $model) is-invalid @enderror"
                wire:model.defer="models.{{ $model }}" {{ $attributes }}
                ...
                @foreach ($options as $option)
                    <option value="{{ $option['value'] }}">
                        {{ Str::contains($option['label'], '::forms') ? __($option['label']) : $option['label'] }}
                    </option>
                @endforeach
            </select>
        </div>
